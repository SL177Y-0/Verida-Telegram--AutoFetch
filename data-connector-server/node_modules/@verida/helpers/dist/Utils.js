 ...
 * @returns
 */
function buildVeridaUri(network, did, contextName, databaseName, itemId, deepAttributes, params) {
    var uri = "verida://".concat(did, "/").concat(network, "/").concat(encodeURI(contextName));
    if (databaseName) {
...
function explodeVeridaUri(uri) {
    ...
    var did = matches[1];
    var network = matches[2];
    var contextName = decodeURI(matches[3]);
    var dbName = matches[4];
    var recordString = matches[5];
